...
export const defaultBusinessTypeProviders: BusinessTypeProvider[] = [
  ...
  { id: 'iso-currency', format: 'iso-currency', detect: ({ value }) => /^[A-Z]{3}$/.test(value.trim()) },
  { id: 'country-iso2', format: 'country-iso2', detect: ({ value }) => /^[A-Z]{2}$/.test(value.trim()) },
  { id: 'country-iso3', format: 'country-iso3', detect: ({ value }) => /^[A-Z]{3}$/.test(value.trim()) },
