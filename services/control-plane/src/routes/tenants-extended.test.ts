...
describe('tenants-extended — HTTP routes', () => {
  ...
  it.each([
    ...
    const payload = await responseJson<any>(response);
  ...
  it('suspend then resume flow', async () => {
    ...
    const suspendPayload = await responseJson<any>(suspendResponse);
    ...
    const resumePayload = await responseJson<any>(resumeResponse);
